# WORK ORDER — ONBOARD CITIZEN-PORTAL

Work Order ID: WO-OBSIDIAN-028
Title: WO-OBSIDIAN-028 — Onboard citizen_portal
Risk Level: LOW
...
Onboard โปรเจกต์ `citizen_portal` ที่ผ่าน recommendation gate จาก WO-OBSIDIAN-024 (Triage Class = `project`, Evidence = `verified`) เข้าสู่ Project Knowledge Vault เป็นโปรเจกต์เดียวที่ bounded หนึ่งงาน โดยใช้ repository truth ปัจจุบัน และรักษาโมเดล authority ของ Vault

ลำดับ onboarding ทีละโปรเจกต์: `thai_stt_app` (WO-025 ✅) → `lumina-studio` (WO-026 ✅) → `lightroom-ai-exposure` (WO-027 ✅) → **`citizen_portal` (WO-028)** → `TalkToClibord` → `AI-Workspace`
...
- Local: `D:\citizen_portal`
- GitHub: `expellirmud-dot/citizen-portal` (remote จาก WO-024 evidence: `expellirmud-dot/citizen-portal`, branch `main`, 37 commits, มี AGENTS.md/CLAUDE.md)

Authority order:

1. Source repository (`D:\citizen_portal`) และไฟล์จริง
...
### 6.2 Project Index
- เพิ่มแถว `[[citizen_portal]]` ในตาราง (พร้อม Description / Local Path / Status / Last Verified)
- ลิงก์กลับ `[[Project Dashboard]]`

### 6.3 Project Dashboard
- ย้าย `citizen_portal` จากหมวด **Discovered — Not Imported** ไป **Imported Projects**
...
1. ห้ามแก้ Source Code ของ `D:\citizen_portal`
...
13. ห้ามเปลี่ยน `04 Work Orders/CURRENT_WORK_ORDER.md` เว้นแต่ Owner อนุญาตให้ activate WO-028 แยกต่างหาก
...
- `04 Work Orders/CURRENT_WORK_ORDER.md` (เปิด pointer → WO-028 ACTIVE, ปิด → CLOSED ให้ครบ proof chain)
- `04 Work Orders/WO-OBSIDIAN-028-ONBOARD-CITIZEN-PORTAL.md` เฉพาะการอัปเดตสถานะ/หลักฐาน closeout เมื่อจบงาน
...
1. exact source repo path/root ถูก resolve (`D:\citizen_portal`, `git rev-parse --show-toplevel`)
...
WO-OBSIDIAN-028 ถือว่า DONE เมื่อครบทุกข้อ:

- [ ] Read-first preflight ผ่าน (`READY`)
- [ ] มี Project Overview เดียวที่ verified สำหรับ `citizen_portal`
...
- **สำหรับการสร้างใบงาน (draft) นี้:** commit ได้หนึ่งครั้งเพื่อสร้างเอกสาร WO-028 เท่านั้น stage เฉพาะไฟล์ WO-028
...
`docs: add WO-OBSIDIAN-028 onboard citizen_portal (planned)`
