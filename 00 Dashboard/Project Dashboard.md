...
> WO-OBSIDIAN-032 expanded adapter coverage to all 11 imported projects. WO-OBSIDIAN-034 integrated the read-only GitHub PR/CI layer (`automation/github_adapter.py`); CI/PR currently render as `unknown`/`null` because the adapter was last run in no-token mode — re-run with `GITHUB_TOKEN` to populate real values.
